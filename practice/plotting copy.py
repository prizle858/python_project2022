...
gryffindor_temp = [] # number of ['Female', 'Male'] in Gryffindor
hufflepuff_temp = [] # number of ['Female', 'Male'] in Hufflepuff
ravenclaw_temp = []  # number of ['Female', 'Male'] in Ravenclaw
slytherin_temp = []  # number of ['Female', 'Male'] in Slytherin

traitlist = [gender, bloodstatus]
...
def func(row):
    if house == 'Gryffindor':
        gryffindor_temp.append(list) # e.g. num_row_df = number of i = 'Gryffindor, j = 'Female'
    elif house == 'Hufflepuff':
        hufflepuff_temp.append(list)
    elif house == 'Ravenclaw':
        ravenclaw_temp.append(list)
    elif house == 'Slytherin':
        slytherin_temp.append(list)
    ...
    for trait in traitlist:
        ...
        if trait == gender: 
            ...
            for i in trait:
                ...
                df_g = hp.loc[(hp['House'] == house) & (hp['Gender'] == i)]
                num_row_df_g = df_g.shape[0] # gives number of the row of the reduced dataframe df
                func(num_row_df_g)

        elif trait == bloodstatus:

            for i in trait:
                df_b = hp.loc[(hp['House'] == house) & (hp['Blood_status'] == i)]
                num_row_df_b = df_b.shape[0] # gives number of the row of the reduced dataframe df
            
    slytherin_gender.append(num_row_df)

gryffindor.append(gryffindor_temp)) # list of number of ['Female', 'Male'] in Gryffindor is appended to the list gryffindor with index of 0
